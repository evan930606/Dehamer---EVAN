...
//
parameter R_DATA_BIT = 9;
parameter G_DATA_BIT = 9;
parameter B_DATA_BIT = 9;
parameter DATA_BIT   = 9;
parameter SRAM_DATA_BIT = 18;
...
begin
    ...
    fans_r = $fopen({"../data/","ans_binary_1_480_640.txt"},"r");
    fout_w = $fopen({"../data/","output_binary_1_480_640.txt"},"w");
end
endtask
...
begin
    ...
    while(while_trig == 1'b0) begin
        ...
        if ((o_rdy == 1'b1) && (i_ack == 1'b1)) begin
            $fwrite(fout_w, "%b\n", o0);
            read = $fscanf(fans_r,"%b", ans);
            if(o_end == 1'b1) while_trig = 1'b1;

            if(o_h == H_NUM-1) begin 
                o_h = 32'b0;
                if(o_w != W_NUM-1) o_w = o_w + 1'b1;
            end
            else           o_h = o_h + 1'b1;
        end
        ...
        if(o_ack == 1'b1 && i_rdy == 1'b1) begin
            if(h == H_NUM-1) begin 
                h = 32'b0;
                if(w < W_NUM-1) w = w + 1'b1;
            end
            else           h = h + 1'b1;
        end
        
        if(h == H_NUM-1 ) i_last = 1'b1;
        else              i_last = 1'b0;

        if(h == H_NUM-1 && w == W_NUM-1) i_end = 1'b1;
        else                             i_end = 1'b0;
